...
function toScreenRect(docRect, zoom, panX, panY) {
    return {
...
function toScreenPt(p, zoom, panX, panY) {
    return { x: p.x * zoom + panX, y: p.y * zoom + panY };
}
// ─── Style helpers ────────────────────────────────────────────────────────────
function applyStyle(ctx, a, zoom) {
    const { style } = a;
...
export function paintAnnotation(ctx, a, opts) {
    ctx.save();
    applyStyle(ctx, a, opts.zoom);
    switch (a.type) {
        case 'highlight':
            paintHighlight(ctx, a, opts);
            break;
        case 'rectangle':
            paintRectangle(ctx, a, opts);
            break;
        case 'ellipse':
            paintEllipse(ctx, a, opts);
            break;
        case 'freehand':
            paintFreehand(ctx, a, opts);
            break;
        case 'text':
            paintText(ctx, a, opts);
            break;
        case 'comment':
            paintComment(ctx, a, opts);
            break;
        case 'line':
            paintLine(ctx, a, opts, false);
            break;
        case 'arrow':
            paintLine(ctx, a, opts, true);
            break;
    }
    ctx.restore();
}
// ─── Individual painters ──────────────────────────────────────────────────────
function paintHighlight(ctx, a, opts) {
...
function paintRectangle(ctx, a, opts) {
    if (a.geometry.kind !== 'rect')
        return;
    const { zoom, panX, panY } = opts;
    const r = toScreenRect(a.geometry.rect, zoom, panX, panY);
    ctx.fillRect(r.x, r.y, r.width, r.height);
    if (a.style.strokeWidth > 0)
        ctx.strokeRect(r.x, r.y, r.width, r.height);
}
function paintEllipse(ctx, a, opts) {
    if (a.geometry.kind !== 'rect')
        return;
    const { zoom, panX, panY } = opts;
    const r = toScreenRect(a.geometry.rect, zoom, panX, panY);
...
function paintFreehand(ctx, a, opts) {
    if (a.geometry.kind !== 'path')
        return;
    const { zoom, panX, panY } = opts;
    const pts = a.geometry.points;
    if (pts.length < 2)
    ...
    if (a.geometry.closed)
        ctx.closePath();
    ctx.stroke();
}
function paintText(ctx, a, opts) {
    if (a.geometry.kind !== 'rect')
        return;
    const { zoom, panX, panY } = opts;
    const r = toScreenRect(a.geometry.rect, zoom, panX, panY);
...
function paintComment(ctx, a, opts) {
    if (a.geometry.kind !== 'point')
        return;
    const { zoom, panX, panY } = opts;
...
function paintLine(ctx, a, opts, withArrow) {
    if (a.geometry.kind !== 'line')
        return;
    const { zoom, panX, panY } = opts;
...
function paintArrowHead(ctx, from, to, color, lineWidth) {
...
export function paintSelectionBorder(ctx, screenRect) {
    ctx.save();
    ctx.strokeStyle = SEL_STROKE;
    ctx.lineWidth = 1.5;
    ctx.setLineDash([5, 3]);
    ctx.strokeRect(screenRect.x - 1, screenRect.y - 1, screenRect.width + 2, screenRect.height + 2);
    ctx.restore();
}
export function paintSelectionHandles(ctx, screenRect) {
...
//# sourceMappingURL=annotation-painters.js.map